...
class SessionManager extends EventEmitter {
  ...
  spawn(opts: { cwd?: string; cmd?: string[] } = {}): SessionMeta {
    const id = randomUUID();
    ...
    try {
      db().prepare(
        `INSERT INTO sessions (id, cwd, cmd, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)`
      ).run(id, cwd, JSON.stringify(cmd), meta.status, now, now);
    } catch (e) {
      console.warn('[sessions] db insert failed:', (e as Error).message);
    }
    logEvent(id, 'session.spawned', { cwd, cmd });

    proc.onData((chunk) => {
      const buf = Buffer.from(chunk, 'utf8');
      appendPty(id, buf);

      // Update replay buffer (keep last REPLAY_MAX bytes)
      const merged = Buffer.concat([internal.replay, buf]);
      internal.replay = merged.length > REPLAY_MAX ? merged.subarray(merged.length - REPLAY_MAX) : merged;
